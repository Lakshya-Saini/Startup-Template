<?php
  ...
  $data = mysqli_query($connect, "SELECT id from users_table WHERE email='$email' AND token='$token'");
  ...
  if($data->num_rows > 0)
  {
    if(isset($_POST['reset']))
    {
        ...
        else
        {
            ...
            mysqli_query($connect, "UPDATE users_table SET password='$hashedpassword' WHERE email='$email' AND token='$token'");
            mysqli_query($connect, "UPDATE users_table SET token='' WHERE email='$email'");
